    ...
    <div className="w-full">
      ...
      <div className="w-full overflow-x-auto hide-scrollbar">
        <motion.div
          ...
          dragElastic={0.15}
        >
          {/* Track the count of each account type as we render */}
          {(() => {
            const typeCount = {};
            return accounts.map((account) => {
              const type = account.accountType;
              typeCount[type] = (typeCount[type] || 0) + 1;
              // Pick style: 0 for first, 1 for second, etc. If more than defined, use last style
              const detailsArr = accountTypeDetails[type] || accountTypeDetails.DEFAULT;
              const details = detailsArr[Math.min(typeCount[type] - 1, detailsArr.length - 1)];
